...
object AudioClassification {
  ...
  val TRAINING_PATH = "data/Train/*"
  val TESTING_PATH = "data/Test/*"

  val AUDIO_CATEGORIES = List("Bell", "Claps", "FireAlarmSound")
...
//    System.setProperty("hadoop.home.dir", "F:\\winutils")
    ...
    val train = sc.textFile("data/Train/Features1.txt")
...
//      println(AUDIO_CATEGORIES.indexOf(parts(0)).toDouble, Vectors.dense(parts(1).split(';').map(_.toDouble)))
    ...
    val test = sc.textFile("data/Test/TestSet.txt")
...
//      println(AUDIO_CATEGORIES.indexOf(parts(0)).toDouble, Vectors.dense(parts(1).split(';').map(_.toDouble)))
    ...
    println("Accuracy is " + metrics.precision)
    // Save and load model
    model.save(sc, "ClassificationModel")
    MongoUpload.insertIntoMongoDB(model.toDebugString);
    val sameModel = DecisionTreeModel.load(sc, "ClassificationModel")
